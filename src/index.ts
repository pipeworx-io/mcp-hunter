...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'Hunter.io');
}
...
async function hunterGet(path: string, params: URLSearchParams): Promise<unknown> {
  ...
  if (!res.ok) {
    // Was `message: <the whole JSON error object, newlines and all>`. Hunter
    // states the reason in `errors[0].details`, which httpErrorMessage lifts
    // out, so the caller gets one sentence instead of a payload (fleet #712).
    return { error: res.status, message: await httpErrorMessage(res, 'Hunter.io') };
  }
...
// Priced off Hunter Growth ($104/mo for 10,000 credits/mo = $0.0104/credit).
// House formula: base = ceil(usd * 1.5 * 15000). Hunter is a SINGLE account (they
// forbid a second), so free callers ride a 50/week slice of the same funded key
// and paid callers a 2,000/week slice — see VENDOR_QUOTAS.PLATFORM_HUNTER_KEY.
export default { tools, callTool, cost: { base: 234 }, provider: 'hunter' } satisfies McpToolExport;
